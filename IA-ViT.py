...
# Define a custom ViT model with learnable tokens for ROI-based voxel activations prediction
class MatrixViTModel(ViTModel):
    def __init__(self, output_dimensions, num_rois, config=None):
        ...
        self.roi_tokens = nn.Parameter(torch.randn(num_rois, config.hidden_size))

        # Initialize linear layers for each ROI token
        self.roi_linear_layers = nn.ModuleList([
            nn.Linear(config.hidden_size, np.prod(output_dimensions)) for _ in range(num_rois)
        ])

        # Initialize the linear layer for the class token
        self.class_linear_layer = nn.Linear(config.hidden_size, np.prod(output_dimensions))

        # Reshaping layer
        self.unflatten = nn.Unflatten(1, output_dimensions)
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.num_rois):
            # Apply linear transformation for each ROI token
            roi_output = self.roi_linear_layers[i](self.roi_tokens[i] + class_token)
            roi_outputs.append(roi_output)

        # Compute voxel activations for the class token
        class_output = self.class_linear_layer(class_token)

        # Return both ROI and class token outputs
        return roi_outputs, class_output
...
class MatrixViTModel(ViTModel):
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.num_rois):
            ...
            query = self.query(self.roi_tokens[i].unsqueeze(0)).unsqueeze(0).repeat(outputs.last_hidden_state.size(0), 1, 1)
            ...
            roi_output = self.roi_linear_layers[i](weighted_sum.squeeze(0))
            roi_outputs.append(roi_output)
...
# Define the dataset and create DataLoader
num_rois = 10  # Number of ROIs
output_dimensions = [512] * num_rois  # Output dimension for each ROI token
some_dataset = ImageVoxelsDataset(nsd_dir, subject, transform=None, target_transform=None, cache_size=0)
dataloader = DataLoader(some_dataset, batch_size=4, shuffle=True, num_workers=0)
