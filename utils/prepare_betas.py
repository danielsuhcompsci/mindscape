...
maskedImages = np.empty((0, np.sum(((prf_atlas > 0) & (prf_atlas < 7))) + np.sum(floc_atlas > 0)))
for session in range(1, 41):
    ...
    for trial in range(750):
        trial_image = np.concatenate((session_image[:,:,:,trial][prf_atlas == 1],
                                      session_image[:,:,:,trial][prf_atlas == 2],
                                      session_image[:,:,:,trial][prf_atlas == 3],
                                      session_image[:,:,:,trial][prf_atlas == 4],
                                      session_image[:,:,:,trial][prf_atlas == 5],
                                      session_image[:,:,:,trial][prf_atlas == 6],
                                      session_image[:,:,:,trial][floc_atlas == 1],
                                      session_image[:,:,:,trial][floc_atlas == 2],
                                      session_image[:,:,:,trial][floc_atlas == 3],
                                      session_image[:,:,:,trial][floc_atlas == 4],
                                      session_image[:,:,:,trial][floc_atlas == 5]))
        maskedImages = np.concatenate((maskedImages, [trial_image]), 0)
    print('session: ', session, 'shape: ', maskedImages.shape)
